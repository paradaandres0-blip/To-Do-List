...
export const Tasks = () => {
  ...
  // Deep-link desde Dashboard: ?id=… (detalle) o ?status=… (listado filtrado)
  useEffect(() => {
    const statusParam = searchParams.get('status');
    const idParam = searchParams.get('id');

    if (isStatus(statusParam)) {
      setFilterStat(statusParam);
      setShowFilters(true);
    }

    if (idParam) {
      const task = getById(idParam);
  ...
  const closeModal = () => {
    setModalOpen(false);
    reset();
    if (searchParams.get('id')) clearQueryParam('id');
  };
  ...
  return (
    <div className="w-full space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          { label: 'Total',       value: counts.total,     color: '#7c3aed', status: 'Todas' as const },
          { label: 'Pendientes',  value: counts.pendiente, color: '#f59e0b', status: 'Pendiente' as const },
          { label: 'En revisión', value: counts.revision,  color: '#2563eb', status: 'En revisión' as const },
          { label: 'Aprobadas',   value: counts.aprobada,  color: '#059669', status: 'Aprobada' as const },
        ].map((s) => (
          <button
            key={s.label}
            type="button"
            onClick={() => {
              setFilterStat(s.status === 'Todas' ? 'Todas' : s.status);
              setShowFilters(s.status !== 'Todas');
              if (s.status === 'Todas') {
                clearQueryParam('status');
              } else {
                setSearchParams({ status: s.status }, { replace: true });
              }
            }}
            className="bg-white rounded-2xl p-4 text-center transition-all hover:shadow-md cursor-pointer"
            style={{
              border: filterStat === s.status || (s.status === 'Todas' && filterStat === 'Todas')
          ...
          <div className="flex flex-wrap gap-3 pt-2" style={{ borderTop:'1px solid #f8fafc' }}>
            ...
            <div className="flex flex-col gap-1">
              ...
              <select
                value={filterStat}
                onChange={(e) => {
                  const value = e.target.value as Status | 'Todas';
                  setFilterStat(value);
                  if (value === 'Todas') clearQueryParam('status');
